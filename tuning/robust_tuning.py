...
def main(opt: str = SGD, base: str = BASE_MODEL, mode: str = SEG):
    ...
    mod, id = tune_val(1, 6, base, SYNTH_D_NEW2, RZESZOW_D, mode, pn, 's', opt, lr) # train: synth/train+val, test: rzesz-val
    # many_eval(mod)
    mod, id = tune_val(id, 6, mod, SYNTH_NEW_RZESZ_D, RZESZOW_D, mode, pn, 'sr', opt, lr/2) # train: synth/train+val + rzesz/test, test: rzesz-val
