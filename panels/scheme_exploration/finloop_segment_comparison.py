"""finloop_segment_comparison.ipynb

# PV segmentation finloop
"""
...
"""
a quick note on areas and their pv share
either:
- photos of equal sizes
- or only adding up the masked areas
...
def sum_pv_segments_sam(pth, model, sam, nazwa="no_info_run", dev=torch.device('cpu'), print_info=False, disp_img=False, display_coef=100):
    pv_area = 0
...
if __name__ == '__main__':
    ...
    with open('finloop_comparison.csv', 'w') as f:
        f.write('dataset,mode,area,time\n')
    ...
    for data_key, dataset in datasets.items():
        ...
        t = time()-t
        print('done', 'finloop-seg', data_key, 'in', t)
        with open('finloop_comparison.csv', 'a') as f:
            f.write(f'{data_key},segment,{area},{t}\n')
        t = time()
        area = sum_pv_segments_sam(dataset, model=model, sam=sam, nazwa=data_key, dev=dev, print_info=False)
        t = time()-t
        print('done', 'finloop with SAM', data_key, 'in', t)
        with open('finloop_comparison.csv', 'a') as f:
            f.write(f'{data_key},SAM,{area},{t}\n')
